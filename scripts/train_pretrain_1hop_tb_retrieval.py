...
from retrieval.data.pre_dataset import PREDatasetNegaMeta, tb_collate
from retrieval.data.pre_dataset import DataPrefetcher
...
def main():
    ...
    if args.momentum and "roberta" not in args.model_name and "longformer" not in args.model_name:
        ...
        logger.info("Pretrain Model Using MomentumRetriever...")
        ...
        logger.info("Pretrain Model Using RobertaMomentumRetriever...")
    elif "roberta" in args.model_name or "longformer" in args.model_name:
        model = RobertaSingleRetriever(bert_config, args)
        logger.info("Pretrain Model Using RobertaSingleRetriever...")
    else:
        model = SingleRetriever(bert_config, args)
        logger.info("Pretrain Model Using SingleRetriever...")
    ...
    eval_dataset = PREDatasetNegaMeta(tokenizer, args.predict_file, args)
    logger.info("Eval Dataset Using PREDatasetNegaMeta...")
    ...
    if args.do_train:
        ...
        train_dataset = PREDatasetNegaMeta(tokenizer, args.train_file, args, train=True)
        logger.info("Training Dataset Using PREDatasetNegaMeta...")
        ...
        for epoch in range(int(args.num_train_epochs)):
            prefetcher = DataPrefetcher(train_dataloader, device)
            ...
            #     batch = move_to_cuda(batch)
                ...
                if (batch_step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if global_step % args.save_checkpoints_steps == 0:
                        ...
                        sys.stdout.flush()
                        sys.stdout.write('\n')
                        logger.info("Saving model with global step =%d" % global_step)
                    ...
                    if args.eval_period != -1 and global_step % args.eval_period == 0:
                        ...
                        if best_mrr < mrr:
                            ...
                            best_mrr = mrr
                batch = prefetcher.next()
                print_str = '\rpretraining: {}/{}, time: '.format(global_step, total)
                period = time.time()-time_start
                m, s = divmod(period, 60)
                h, m = divmod(m, 60)
                print_str += "%d:%02d:%02d " % (h, m, s)
                period = period/global_step*total
                m, s = divmod(period, 60)
                h, m = divmod(m, 60)
                print_str += "<%d:%02d:%02d   " % (h, m, s)
                sys.stdout.flush()
