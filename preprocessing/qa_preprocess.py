...
def prepare_training_data_random_nega(ziped_data, table_path, request_path, max_passages=3, max_tokens=360,
    ...
    for i, row in enumerate(contents):
        ...
        if i in positive_row_index:
            ...
            # ranked_passages = rank_doc_tfidf(data['question'], gt_passages)
            # ranked_passages = ranked_passages if len(
            #     ranked_passages) <= max_passages else ranked_passages[:max_passages]
            # ranked_raw_passages = [' '.join(para['passage'].split(' ')) for para in ranked_passages]
            passages.append(ranked_passages)
            passages_index.append({'all-index': _index, 'sample-index': [p['passage_id'] for p in ranked_passages]})
            # final_index = random.sample(_index, max_passages) if len(_index) >= max_passages else _index
            # all_raw_passages.append([requested_documents[index] for index in _index])
            # raw_passages = [requested_documents[index] for index in final_index]
            #
            # trunk_passages = [' '.join(para.split(' ')[:max_tokens]) for para in raw_passages]
            # passages.append(trunk_passages)
            # passages_index.append({'all-index':_index, 'sample-index':final_index})
            passages_node_index.append([data['answer-node'][_i] for _i, (x,y) in enumerate(positive_index) if x==i])

            # # junjie's version of preprocess
            # raw_passages = [requested_documents[index] for index in _index]
            # all_raw_passages.append(raw_passages)
            # tfidf_query = [data['question'] + ' ' + ' '.join(header) + ' ' + ' '.join(row)]
            # # tfidf_query = [' '.join(header) + ' ' + ' '.join(row)]
            # # tfidf_query = [data['question']]
            # ranked_passages = get_sorted_passages(tfidf, tfidf_query, _index, raw_passages)
            # passages.append([item[1] for item in ranked_passages])
            # sorted_psg_index = [item[0] for item in ranked_passages]
            # passages_index.append({'all-idx': _index, 'sort-idx': sorted_psg_index})
            # passages_node_index.append([data['answer-node'][_i] for _i, (x,y) in enumerate(positive_index) if x==i])
        else:
            # 如果answer node中没有这一行，那么passages就要么随机取这一行max_passages个，要么就取这一行的所有passages
...
if __name__ == '__main__':
    """
        ...
        python qa_preprocess.py \
            --split dev \
            --retrieval_results_file ../preprocessed_data/qa/data4qa_v2/dev_output_k100_tfidf20.json
        
        python qa_preprocess.py \
            --split train \
	        --retrieval_results_file ../preprocessed_data/qa/data4qa_v2/train_output_k100.json
    """
    ...
    parser.add_argument("--qa_save_path", type=str, default=f'{basic_dir}/preprocessed_data/qa/train_preprocessed_gtmodify.json',)
    # parser.add_argument("--retrieval_results_file", type=str, default='/home/t-wzhong/v-wanzho/ODQA/data/preprocessed_data/qa/data4qa_v2/dev_output_k100.json',
    parser.add_argument("--retrieval_results_file", type=str, default=r'{basic_dir}/preprocessed_data/qa/data4qa_v2/dev_output_k100.json',
    ...
    if args.split in ['train', 'dev']:
        ...
        if not os.path.exists(args.qa_save_path) or args.reprocess:
            ...
            if args.add_links: # for preprocessing of reasoning chain
                new_qa_results = []
                for instance in tqdm(qa_results):
                    new_instance = copy.deepcopy(instance)
                    for tb_idx, tb in enumerate(instance['retrieved_tbs']):
                        tb_row = tb['table'][1][0]
                        # table_block_id = "{}_{}".format(tb['table_id'], tb['row_id'])
                        chain = [[]] * len(tb_row)
                        if data_links.get(tb['table_id'], None):
                            table = data_links[tb['table_id']]
                            row = table['data'][tb['row_id']]
                            if len(row) == len(tb_row):
                                chain = [item[1] for item in row]
                            else:
                                row_dict = {item[0]:item[1] for item in row}
                                try:
                                    chain = [row_dict[key] for key in tb_row]
                                except:
                                    chain = [[]] * len(tb_row)
                                    logger.info("tb_row: {}, table_row: {}".format(tb_row, [item[0] for item in row]))
                        else:
                            logger.info("not in tables: {}".format(tb['table_id']))
                        new_instance['retrieved_tbs'][tb_idx]['chain'] = chain
                    new_qa_results.append(new_instance)
                qa_results = new_qa_results
            ...
            #     result = prepare_training_data_random_nega(item,tbid2docs,table_path=table_path, request_path=request_path, max_passages=args.max_passages, max_tokens=args.max_tokens, max_cell_tokens=args.max_cell_tokens)
            ...
            #     print('Saving the output to {}'.format(qa_save_path))
            # print(json.dumps(qa_results[0]['positive_table_blocks'],indent=4))
            # #     output_compare.append(prepare_training_data_random_nega(item,tbid2docs,table_path=table_path, request_path=request_path,
            # #                                                      max_passages=args.max_passages,
            # #                                                      max_tokens=args.max_tokens,
            # #                                                      max_cell_tokens=args.max_cell_tokens)['output_compare'])



            # check_file = open('/home/t-wzhong/v-wanzho/ODQA/qa_log/dev_gt_retrieve_compare.json', 'w', encoding='utf8')
            # json.dump(output_compare,check_file,indent=4)
            # print('Saving the output to {}'.format('/home/t-wzhong/v-wanzho/ODQA/qa_log/dev_gt_retrieve_compare.json'))
